...
//Login ...
func (uc UserUC) Login(data request.UserLoginRequest) (res viewmodel.UserLoginVM, err error) {
	ctx := "UserUC.Login"

	userModel := model.NewUserModel(uc.DB)
	results, err := userModel.FindByPhoneNumber(data.PhoneNumber)
	if err != nil {
		logruslogger.Log(logruslogger.WarnLevel, data.PhoneNumber, ctx, "phone_or_password_not_match", uc.ReqID)
		return res, err

	}
	if results.PhoneValidatedAt.String == "" {
		logruslogger.Log(logruslogger.WarnLevel, data.PhoneNumber, ctx, "phone_not_verified", uc.ReqID)
		return res, errors.New(helper.PhoneNotValidated)
	}
	if results.Pin.String == "" {
		logruslogger.Log(logruslogger.WarnLevel, data.PhoneNumber, ctx, "pin_unset", uc.ReqID)
		return res, errors.New(helper.PinUnset)
	}

	authUc := AuthUC{ContractUC: uc.ContractUC}
	passwordInput, err := authUc.DecryptedOnly(data.Pin)
	if err != nil {
		logruslogger.Log(logruslogger.WarnLevel, err.Error(), ctx, "query", uc.ReqID)
		return res, err
	}
